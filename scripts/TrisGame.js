...
function buildGameUI()
{
	...
	var sqSize = (BOARD_SIZE - (BOARD_BORDER * (NUMBER_SQUARES_ROW + 1))) / NUMBER_SQUARES_ROW;
	
	// vẽ từng ô vuông 1
	for (var i = 0; i < NUMBER_SQUARES_ROW * NUMBER_SQUARES_ROW; i++)
	{
		// định nghĩa ô vuông
		var square = new createjs.Container();
		
		// tọa độ
		square.x = i % NUMBER_SQUARES_ROW * (sqSize + BOARD_BORDER) + BOARD_BORDER;
		square.y = Math.floor(i / NUMBER_SQUARES_ROW) * (sqSize + BOARD_BORDER) + BOARD_BORDER;
...
function makeMove(evt)
{
	...
	var x = square.id % NUMBER_SQUARES_ROW + 1;
	var y = Math.floor(square.id / NUMBER_SQUARES_ROW) + 1;
...
function moveReceived(params)
{
	...
	{
		var square = squares[(params.get("y") - 1) * NUMBER_SQUARES_ROW + (params.get("x") - 1)];
...
function setSpectatorBoard(params)
{
	...
	for (var y = 0; y < boardData.size(); y++)
	{
		...
		{
			var square = squares[y * NUMBER_SQUARES_ROW + x];
